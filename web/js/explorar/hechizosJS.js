...
Escuela.addEventListener('change', function () {
    ...
    let urlDestinoOrden = "/TFG/Explorar/hechizos?escuela=" + valorSeleccionado + "&nivel=" + Nivel.value + "&clase=" + Clase.value + "&pag=" + pag;
    window.location.href = urlDestinoOrden;
});
Nivel.addEventListener('change', function () {
    let valorSeleccionado = Nivel.value;
    let urlDestinoOrden = "/TFG/Explorar/hechizos?escuela=" + Escuela.value + "&nivel=" + valorSeleccionado + "&clase=" + Clase.value + "&pag=" + pag;
    window.location.href = urlDestinoOrden;
});
Clase.addEventListener('change', function () {
    let valorSeleccionado = Clase.value;
    let urlDestinoOrden = "/TFG/Explorar/hechizos?escuela=" + Escuela.value + "&nivel=" + Nivel.value + "&clase=" + valorSeleccionado + "&pag=" + pag;
    window.location.href = urlDestinoOrden;
});
...
if (pag > 2) {
    Binicio.addEventListener('click', function () {
        urlDestinoPag = "/TFG/Explorar/hechizos?escuela=" + Escuela.value + "&nivel=" + Nivel.value + "&clase=" + Clase.value + "&pag=" + "1";
...
if (pag > 1) {
    BAnterior.addEventListener('click', function () {
        urlDestinoPag = "/TFG/Explorar/hechizos?escuela=" + Escuela.value + "&nivel=" + Nivel.value + "&clase=" + Clase.value + "&pag=" + (pag - 1);
        window.location.href = urlDestinoPag;
    });
}
//Pag posterior
